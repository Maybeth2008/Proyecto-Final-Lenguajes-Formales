...
public class Main{
	public static void main(String[] args) {
		...
		AnalisisEstatico analisis=new AnalisisEstatico();
		ArrayList<AST> reglas = new ArrayList<>();
		...
		for(int j=0; j<v; j++){
		    ...
		    if(vax.contains("=")){
		        ...
		        interprete.hechos.add(vax.trim());
		    }
		...
		for(String hecho : interprete.hechos){
            System.out.println(hecho);
        }
        analisis.redundancia(reglas);
        analisis.conflictos(reglas);
        analisis.inactivo(reglas);
